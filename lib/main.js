...
function switch_to_buffer(x) {
	...
	if (active_buffer)
		active_buffer.classList.remove('active')
	active_buffer = buffer
	active_buffer.classList.add('active')
}
...
function on_key_down(e) { return this.active_buffer.on_key_down(e) }
...
function* filter(f, xs) {
	let i = 0
	for (const x of xs) if (f(x, i++, xs)) yield x
}

function* map(f, xs) {
	let i = 0
	for (const x of xs) yield f(x, i++, xs)
}

function each(f, xs) {
	let i = 0
	for (const x of xs) f(x, i++, xs)
	return xs
}

function* slice(a, b, xs) {
	for (let i = a; i < b; i++)
		yield xs[i]
}

function reduce(f, i, xs) {
	for (const x of xs) i = f(x, i)
	return i
}
